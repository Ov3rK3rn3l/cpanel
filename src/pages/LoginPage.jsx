import React, { useState } from 'react';
    ...
    const LoginPage = () => {
      ...
      const { login, supabase, setUserRole } = useAuth(); // Adicionado setUserRole para atualizar o contexto
      const { toast } = useToast();

      const logoUrl = "/logo-gerr-shield.png"; 
        ...
        try {
          ...
          if (loginData.user) {
            ...
            if (profileError && profileError.code !== 'PGRST116') {
                console.error("Erro ao buscar perfil do usuário após login:", profileError.message);
                toast({
                  title: "Erro de Perfil",
                  description: "Não foi possível buscar seu perfil. Contate um admin.",
                  variant: "destructive",
                });
                // Não impede o login, mas o userRole no contexto pode não ser atualizado imediatamente
            } else if (userProfile) {
                role = userProfile.role;
            }
            
            setUserRole(role); // Atualiza a role no AuthContext
            ...
            } else if (['admin', 'moderador', 'recrutador'].includes(role)) {
              navigate('/admin/dashboard', { replace: true });
            } else {
              navigate('/', { replace: true }); 
            }
          } else {
            // Isso não deveria acontecer se login() não lançar erro e retornar user null
            // Mas por segurança:
            toast({
              title: "Erro no Login",
          ...
          >
            <Card className="w-full max-w-md shadow-2xl bg-card/80 backdrop-blur-md border-border/50 glassmorphic">
              <CardHeader className="text-center">
                ...
                >
                  {logoUrl ? (
                    <img-replace src={logoUrl} alt="Logo Clã GERR" className="h-20 w-20 animate-float" />
                  ) : (
