...
const FeaturedMembersCard = () => {
  ...
  const fetchFeaturedMembers = useCallback(async () => {
    ...
    try {
      const { data, error } = await supabase.rpc('get_featured_members');
    ...
    return (
      ...
      >
        {featuredMembers.map((member, index) => (
          <MotionCard
            key={member.id}
            className="glassmorphic-dark border-primary/20 overflow-hidden group relative"
          ...
          >
            ...
            <CardContent className="pt-8 flex flex-col items-center text-center relative z-10">
              <motion.div
                ...
                className="w-28 h-28 rounded-full overflow-hidden border-4 border-primary/50 shadow-lg mb-4 bg-slate-700 flex items-center justify-center"
