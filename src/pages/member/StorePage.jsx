...
import { Loader2, Store, Coins, Shield, Package, Lock, ShoppingCart, CheckCircle, KeyRound } from 'lucide-react';
...
const StorePage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const itemsPromise = supabase.from('store_items').select('*, store_item_keys(count)').eq('is_active', true).order('price', { ascending: true });
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div key={item.id} variants={cardVariants} initial="hidden" animate="visible" custom={i}>
                  <Card className={`h-full flex flex-col glassmorphic-dark-glow group ${isSoldOutByLimit || isSoldOutByKey ? 'opacity-60' : ''}`}>
