...
const RankingCard = ({ currentMemberId }) => {
  ...
  const fetchRanking = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.rpc('get_ranking_with_effects', { p_limit: 10 });
  ...
  return (
    <Card className="glassmorphic border-primary/30 h-full flex flex-col">
      ...
      <CardContent className="custom-scrollbar space-y-2 max-h-80 overflow-y-auto flex-grow pr-2">
        {ranking.length === 0 ? (
          ...
          <AnimatePresence>
            {ranking.map((member, index) => (
              <motion.div
                ...
                className={cn(
                  `p-3 bg-card/60 rounded-lg border flex items-center gap-4 transition-all duration-300`,
