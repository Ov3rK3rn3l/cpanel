...
import { Loader2, Store, Coins, Shield, Package, Tag, Lock, ShoppingCart, CheckCircle } from 'lucide-react';
...
const StorePage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const itemsPromise = supabase.from('store_items').select('*').eq('is_active', true).order('price', { ascending: true });
  ...
  const getPurchaseStatus = (item) => {
    if (!memberData) return { canBuy: false, reason: 'Carregando dados do membro...' };
    ...
    if (memberPatenteOrder < requiredPatenteOrder) {
      return { canBuy: false, reason: `Requer Patente: ${item.required_patente}` };
    }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="container mx-auto py-8 px-2 sm:px-4">
      <div className="text-center mb-10">
        ...
        >
          <Store className="h-10 w-10 mr-4" /> Loja G.E.R.R
        </motion.h1>
        <motion.p 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <AnimatePresence>
            {items.map((item, i) => {
              ...
              const purchasesMade = purchaseHistory[item.id] || 0;
              const isSoldOut = item.purchase_limit > 0 && purchasesMade >= item.purchase_limit;

              return (
                <motion.div key={item.id} variants={cardVariants} initial="hidden" animate="visible" custom={i}>
                  <Card className={`h-full flex flex-col glassmorphic-dark-glow group ${isSoldOut ? 'opacity-60' : ''}`}>
                    <CardHeader className="relative">
                      <div className="aspect-video bg-card/80 rounded-lg mb-4 flex items-center justify-center overflow-hidden">
                        {item.image_url ? (
                          <img-replace src={item.image_url} alt={item.name} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                        ) : (
                        ...
                        <AlertDialogTrigger asChild>
                          <Button className="w-full btn-primary-dark" disabled={!canBuy || isPurchasing === item.id}>
                            {isPurchasing === item.id ? <Loader2 className="h-5 w-5 animate-spin" /> : 
                             isSoldOut ? <><CheckCircle className="h-5 w-5 mr-2"/>Adquirido</> :
