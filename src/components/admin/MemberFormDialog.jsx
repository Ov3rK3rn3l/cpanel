import React, { useState, useEffect, useCallback } from 'react';
    ...
    import { PATENTE_OPTIONS, STATUS_OPTIONS, JOGO_PRINCIPAL_OPTIONS, YES_NO_OPTIONS, YES_NO_OPTIONS_VALUES, formatDateForInput } from './members/utils';

    const initialFormData = {
      codinome: '',
      discord_id: '',
      email: '',
      patente_atual: PATENTE_OPTIONS[0]?.value || '',
    ...
    const MemberFormDialog = ({ isOpen, onOpenChange, member, onSave, onMarkAsLeft }) => {
        ...
        if (isOpen) {
          if (member) {
            setFormData({
              ...
              advertencias: member.advertencias || [],
            });
      ...
      const handleSubmit = async (e) => {
        ...
        ['ultima_presenca', 'penultima_presenca', 'data_ingresso', 'data_saida'].forEach(dateField => {
          if (!memberPayload[dateField] || memberPayload[dateField] === '') {
            ...
            // Garante que a data esteja no formato YYYY-MM-DD antes de enviar para o Supabase
      ...
      const handleAdicionarAdvertencia = () => {
        toast({
          title: "Funcionalidade em Desenvolvimento",
          description: "🚧 A gestão detalhada de advertências diretamente aqui ainda não foi implementada. Você pode solicitar esta funcionalidade! 🚀",
          variant: "default",
          duration: 5000,
        });
      };
              ...
              <form onSubmit={handleSubmit} className="py-4 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                  ...
                  <div>
                    ...
                    <Input id="promocao_status" name="promocao_status" value={formData.promocao_status} onChange={handleChange} className="input-dark mt-1" placeholder="Ex: Promovido para..." />
                  </div>
                  <div>
                ...
                {member && (
                  <div className="mt-6 border-t border-border pt-6">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <Button type="button" variant="outline" size="sm" onClick={handleAdicionarAdvertencia} className="btn-destructive-outline">
                      ...
                      <ul className="space-y-2 text-sm text-muted-foreground max-h-32 overflow-y-auto pr-1">
                        {formData.advertencias.map((adv, index) => (
                          <li key={index} className="p-2 border rounded-md bg-card/50">
                            <strong>{adv.tipo}</strong> ({formatDateForInput(adv.data, 'dd/MM/yyyy')}): {adv.motivo}
